...
source: <https://github.com/Continuum-AI-Corp/OrcaPromptVault/blob/main/Kilo-Code/kilo-auto-free-system-prompt-2026-09-05.md>

reproduce: `node capture/capture.mjs kilo` - captured off the wire with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) (Apache-2.0), a local proxy in front of the model API recording the request the harness actually sent. Kilo Code in `run` mode on the free tier, capture profile `kilo/kilo-auto/free`; 11,321 characters and 13 tool definitions as sent. Kilo's origin cannot be moved with an environment variable, so this capture terminates the TLS Kilo establishes itself. Machine-identifying strings are replaced with `{PLACEHOLDER}` tokens; nothing else is reworded or reordered.
